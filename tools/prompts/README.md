...
text prompt → Nano Banana (image) → Hunyuan3D (image-to-3D) → .glb
                                                                ↓
                                       compress with @gltf-transform/cli
                                                                ↓
                                  write to  assets/models/<name>.glb
                                                                ↓
                                            three.js auto-loads it
```

Compression is the same step used by `tools/blender/compress_drone.sh` —
see [`../README.md`](../README.md) for the exact command.
